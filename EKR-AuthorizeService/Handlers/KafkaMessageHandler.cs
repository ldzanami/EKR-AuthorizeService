...
namespace EKR_AuthorizeService.Handlers
{
    public class KafkaMessageHandler(IAuthService authService,
                                     ...
                                     IAESEncryptorService AESEncryptorService) : IKafkaMessageHandler
    {
        ...
        {
            var package = JsonSerializer.Deserialize<GeneralPackageTemplate>(message)!;

            var decrypted = _RSADecryptorService.Decrypt(package.AESKey, package.IV, package.Content);

            if (package.Type == "register")
            {
                var result = await _authService.RegisterAsync(JsonSerializer.Deserialize<RegisterRequestDto>(decrypted.Content)!);
                await SentToKafka(result, decrypted.AesKey, package.IV);
            }
            else if (package.Type == "authorize")
            {
                var result = await _authService.AuthorizationAsync(JsonSerializer.Deserialize<AuthorizationDto>(decrypted.Content)!);
                await SentToKafka(result, decrypted.AesKey, package.IV);
            }
            else if (package.Type == "refresh")
            {
                var result = await _authService.RefreshAsync(decrypted.Content);
                await SentToKafka(result, decrypted.AesKey, package.IV);
            }
            else if (package.Type == "revoke")
            {
                await _authService.RevokeSessionAsync(Guid.Parse(decrypted.Content));
                await SentToKafka("OK", decrypted.AesKey, package.IV);
            }
            else if (package.Type == "revoke-others")
            {
                await _authService.RevokeOtherSessionsAsync(JsonSerializer.Deserialize<RevokeOtherSessionsDto>(decrypted.Content)!);
                await SentToKafka("OK", decrypted.AesKey, package.IV);
            }
            else if (package.Type == "revoke-all")
            {
                await _authService.RevokeAllSessionsAsync(Guid.Parse(decrypted.Content));
                await SentToKafka("OK", decrypted.AesKey, package.IV);
            }
            else if (package.Type == "get-active-sessions")
            {
                var result = await _authService.GetActiveUserSessionsAsync(Guid.Parse(decrypted.Content));
                await SentToKafka(result, decrypted.AesKey, package.IV);
            }
        }

        private async Task SentToKafka(object result, byte[] aesKey, byte[] IV)
        {
            var answer = Encoding.UTF8.GetString(_AESEncryptorService.Encrypt(JsonSerializer.Serialize(result), aesKey, IV));
            await _kafkaProducerService.GiveAnswerAsync(answer);
        }
    }
}
